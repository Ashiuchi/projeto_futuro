...
```mermaid
graph TD
    A["⭐ Tríplice Coroa (BACEN • ANPD • BB)"] --> B["Segurança da Informação & Gestão de Riscos"]
    A --> C["LGPD & Privacidade (ISO 27701)"]
    A --> D["Engenharia de Software & DevSecOps"]
    A --> E["Infraestrutura em TI & Nuvem (K8s/Azure)"]
    A --> F["Ciência de Dados & IA"]
    A --> G["Bancos de Dados & SQL"]
    A --> H["Língua Portuguesa"]

    I["Sinergia Federal (BACEN + ANPD)"] --> J["Direito Constitucional (Art. 5º / Art. 37)"]
    I --> K["Direito Administrativo (Lei 8.112 / Lei 9.784)"]
    I --> L["Licitações e Contratos (Lei 14.133/2021)"]

    M["Exclusivos"] --> N["Micro & Macroeconomia (BACEN)"]
    M --> O["Conhecimentos Bancários & Inglês (BB)"]
```
